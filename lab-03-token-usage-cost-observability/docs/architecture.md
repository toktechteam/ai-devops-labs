...
## Observability
- Structured JSON logs include request metadata, token usage, and estimated cost.
- EMF metrics include token counts, estimated cost, and latency breakdowns.
- API Gateway access logs are enabled on the `/prod` stage.
- A CloudWatch dashboard visualizes tokens, cost, request count, and latency.
