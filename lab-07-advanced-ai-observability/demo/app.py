...
DEFAULT_TRACEPARENT = os.environ.get("TRACEPARENT", "")


def _post_json(api_url: str, token: str, payload: dict, traceparent: str | None) -> tuple[int, dict]:
...
def index():
    return render_template(
        ...
        default_traceparent=DEFAULT_TRACEPARENT,
    )
...
def invoke():
    ...
    traceparent = (data.get("traceparent") or "").strip() or None
    ...
    status, response = _post_json(api_url, token, payload, traceparent)
